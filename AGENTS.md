# inari-cli — Agent Guide

The `inari` CLI: login (OIDC device flow), cluster/catalog/resource ops, agent install, extension scaffolding (`inari extension init`) (plan §6 #9). `inari cluster kubeconfig <id>` prints a secret-free kubelogin exec-credential kubeconfig from the server's cluster access-info endpoint (plan §7.2).

Stack: Go, cobra

## Key architecture constraints
- v1 scope is **core flows only**: login, cluster register, catalog, deploy, resources — full console parity is explicitly NOT a v1 goal (§11/4).
- Auth: OIDC device flow against the platform Keycloak `inari` realm; tokens carry org/team claims (§5.4).
- Consume REST/OpenAPI contracts from the `inari-api` repo (generated Go client); pin versions (§6).
- Binary releases via goreleaser (GitHub Release archives; brew/scoop planned — see Conventions) (§6).

## Conventions
- Conventional Commits; SemVer releases; container images/artifacts cosign-signed (once CI exists).
- Releases are driven by **release-please in PR-only mode** (`.github/workflows/release-please.yml`, `release-type: go`): pushes to `main` open/update a Release PR (version bump + `CHANGELOG.md`). Merging the Release PR triggers `.github/workflows/release.yml`, which creates the tag + GitHub Release and runs goreleaser (`.goreleaser.yml`) to publish binary archives + checksums. Goreleaser is invoked from `release.yml` on push to `main` — never via tag-push triggers. Goreleaser requires the checked-out commit to carry the release tag, so there is **no manual re-publish escape hatch**: to (re)publish artifacts, cut a new patch release via release-please (merge a `fix` commit, then the Release PR).
- Pre-1.0 versioning: breaking changes bump minor, features bump minor, fixes bump patch (`bump-minor-pre-major` in `release-please-config.json`).
- Do not edit `CHANGELOG.md` by hand; it is generated by release-please from Conventional Commits.
- brew/scoop tap publishing is currently **disabled** (removed from `.goreleaser.yml`): the planned tap repos `7K-Inari/homebrew-tap` / `7K-Inari/scoop-bucket` don't exist and there is no working `HOMEBREW_TAP_GITHUB_TOKEN`. Follow-up: create the tap repos, add the token secret, and restore publishing with `brews:` (formula, for a CLI) and `scoops:`.
- Write tests for new behavior; keep changes minimal and focused.
- Canonical architecture & development plan: https://github.com/7K-Inari/inari-docs/blob/main/docs/architecture/inari-platform-plan.md (section references below point into it).

## Platform design principles (apply everywhere)
1. Tenant-aware to the core — every object carries a tenant ID; every API decision is tenant-scoped.
2. Zero tenant credentials on the hub — no tenant kubeconfigs or cloud keys in the control plane.
3. Pull, never push — agents dial out; the control plane never initiates connections into tenant networks.
4. Desired state, eventually reconciled — GitOps/CR-based mutations, not imperative RPCs.
5. The catalog is a projection of reality — capabilities are discovered, not declared.
6. Small kernel, everything else extension.
7. Modular monolith first — strict internal module boundaries.
