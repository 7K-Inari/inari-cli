...
- Releases are driven by **release-please in PR-only mode** (`.github/workflows/release-please.yml`, `release-type: go`): pushes to `main` open/update a Release PR (version bump + `CHANGELOG.md`). Merging the Release PR triggers `.github/workflows/release.yml`, which creates the tag + GitHub Release and runs goreleaser (`.goreleaser.yml`) to publish binary archives + checksums. Goreleaser is invoked from `release.yml` on push to `main` — never via tag-push triggers. `workflow_dispatch` on `release.yml` is the manual escape hatch to re-publish artifacts for the latest existing tag/release.
